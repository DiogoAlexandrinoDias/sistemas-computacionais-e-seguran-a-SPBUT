...
{ private byte []  textoCifrado;
 ...
 public void gerarChave (File fSim)
 throws IOException, NoSuchAlgorithmException, InvalidAlgorithmParameterException,
   CertificateException, KeyStoreException
   {//Gera uma chave simetrica de 128:
   KeyGenerator kg = KeyGenerator.getInstance("AES");
   kg.init(128);
   SecretKey sk = kg.generateKey();
   // Grava a chave simetrica em formato Seralizado
 ...
 throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException,
         ...
         InvalidAlgorithmParameterException, IOException, ClassNotFoundException
           {ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fSim));
            ...
            ois.close();
            Cipher aescf = Cipher.getInstance ("AES/CBC/PLCSS5Padding");
        ...
        public void gerarDecifrado(byte[] texto,  File fSim)
        throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException,
               IllegalBlockSizeException, BadPaddingException,
               InvalidAlgorithmParameterException, IOException, ClassNotFoundException
               {ObjectInputStream ois = new ObjectInputStream(new FileInputStream (fSim));
                 SecretKey iSim = (SecretKeySpec) ois.readObject();
                ois.close();
